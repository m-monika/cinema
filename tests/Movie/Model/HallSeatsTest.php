...
class HallSeatsTest extends TestCase
{
    ...
    {
        $hallSeat = new HallSeats(new Seat(1, 1, 1, false));
        $this->assertFalse($hallSeat->isSeatAvailable(1, 1, 1));
    }

    public function testNotTakenSeat(): void
    {
        $hallSeat = new HallSeats(new Seat(1, 1, 1, true));
        $this->assertTrue($hallSeat->isSeatAvailable(1, 1, 1));
    }

    public function testSeatDoesntExists(): void
    {
        $hallSeat = new HallSeats(new Seat(1, 1, 1, false));
        $this->assertFalse($hallSeat->isSeatAvailable(1, 1, 2));
    }
}
